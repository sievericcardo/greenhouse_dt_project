...
For the data collector, a `.img` file can be retrieved [here](https://figshare.com/s/a97674ae14d9631d293b). It can be saved on a 32 GB microSD card with the `dd` utilities. The image is based on the Raspberry Pi OS 64bit version. The image is configured to use the following credentials:
...
For the actuator, a `.img` file can be retrieved [here](https://figshare.com/s/00c583777ba6258c3d4c). It can be saved on a 32 GB microSD card with the `dd` utilities. The image is based on the Raspberry Pi OS 64bit version. The image is configured to use the following credentials:
